internal class Part2
{
	...
	private int W;
	private int H;
	private const int NumMovesPerCheat = 20;
	...
	{
		foreach (var cheatCount in Cheats.OrderByDescending(pair => pair.Value))
		{
			if (cheatCount.Key > 50)
				Console.WriteLine($"There are {cheatCount.Value} cheats that save {cheatCount.Key} picoseconds.");
		}
	}
	...
	private void ExecuteSearch()
	{
		...
		while (!(x == EndX && y == EndY))
		{
			for (var nx = x - NumMovesPerCheat; nx <= x + NumMovesPerCheat; nx++)
			{
				for (var ny = y - NumMovesPerCheat; ny <= y + NumMovesPerCheat; ny++)
				{
					var d = Math.Abs(ny - y) + Math.Abs(nx - x);
					if (d > NumMovesPerCheat)
						continue;
					if (!InBounds(nx, ny))
						continue;
					if (Grid[nx, ny] == int.MaxValue)
						continue;
					var savings = Grid[nx, ny] - Grid[x, y] - d;
					if (savings > 0)
					{
